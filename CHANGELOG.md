...
- Ran `npm audit fix` to pick up patched transitive dependencies.
...
- Host-clipboard tests: capability capture from `host:init`, `readClipboardViaHost` request/response matching and timeout, bridge inertness without the capability, activation when the capability arrives, and host-mediated copy/cut/paste against a fake extension host.
...
- Added unit tests for the clipboard field helpers (`clipboardFallback`) and the host-mode clipboard bridge (keyboard shortcuts, context menu paste, disabled states, Escape-to-close, and no-op outside host mode).
...
- Column card count badge now correctly animates to the right position when drag handles are hidden — it previously reserved space for the grip button even when it wasn't shown.
...
- Run `npm audit fix` to apply security patches to transitive dependencies
...
- Bump dev dependency patch versions in lockfile (`brace-expansion`, `picomatch`, `smol-toml`, `yaml`)
...
- Remove redundant visual regression test
...
- Make keyboard shortcut hint in bottom bar clickable to open the command palette
...
- Settings modal sections remember their open/closed state across sessions via IndexedDB
...
- Enrich initial board with feature-rich starter cards — new users see card types (Feature, Chore), due dates, markdown checklists, descriptions, and realistic column history out of the box

### Documentation

- Add enriched initial board item to roadmap shipped section
...
- Remove bullet points from checklist items in markdown preview
