...
function createRenderer(interactive: boolean): RendererObject {
  let checkboxIndex = 0;
  return {
    ...
    checkbox({ checked }) {
      ...
      return `<input type="checkbox" ${checked ? 'checked="" ' : ""}${disabledAttr}data-checkbox-index="${idx}" class="mr-1.5 align-middle${interactive ? " cursor-pointer" : ""}"> `;
    },
...
function parse(content: string, interactive: boolean): string {
  const md = new Marked({
    renderer: createRenderer(interactive),
    gfm: true,
    async: false,
  });
...
export function MarkdownPreview({
  ...
  const html = useMemo(() => {
    ...
    return parse(content, !!onCheckboxToggle);
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent<HTMLDivElement>) => {
      ...
      if (
        ...
        target.dataset.checkboxIndex !== undefined
      ) {
        e.preventDefault();
        e.stopPropagation();
        onCheckboxToggle(Number(target.dataset.checkboxIndex));
      }
    },
    [onCheckboxToggle],
  );
  ...
  return (
    // eslint-disable-next-line jsx-a11y/click-events-have-key-events, jsx-a11y/no-static-element-interactions -- checkbox clicks are handled via delegation; parent element provides keyboard/role semantics
    <div
      className={`text-sm ${tc.text} ${className}`}
      dangerouslySetInnerHTML={{ __html: html }}
      onClick={onCheckboxToggle ? handleClick : undefined}
    />
  );
}
